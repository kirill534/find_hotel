...
from keyboards.inline.calendar_buttons import CallbackData, Calendar
import handlers.custom_handlers.user_commands
        ...
        with bot.retrieve_data(call.message.chat.id) as data:
            if 'checkInDate' in data:
                ...
                if int(select_date) > checkin:
                    logger.info(f'Ввод и сохранение даты выезда. {day}:{month}:{year}')
                    ...
                    if data['flag_checking']:
                        ...
                        data['price_min'] = 0
                        data['price_max'] = 0
                        if data['command_buttons'] == 'bestdeal':
                            bot.set_state(call.message.chat.id, UserInputState.priceMin)
                            bot.send_message(call.message.chat.id, f'Введите минимальную стоимость отеля в {data["currencies_sign"]}.')
                        else:
                            keyboards.inline.check_info.buttons_info(call.message, data)
                else:
                    bot.send_message(call.message.chat.id, 'Дата выезда должна быть больше даты заезда! '
                                                           'Повторите выбор даты!')
                    handlers.custom_handlers.user_commands.my_calendar(call.message, 'выезда')
            else:
                if int(select_date) >= int(now):
                    logger.info(f'Ввод и сохранение даты заезда. {day}:{month}:{year}')
